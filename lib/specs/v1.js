...
const docsBaseUrl = `https://docs.ghost.org/api/handlebars-themes/`;
...
knownHelpers = [
    ...
    'log', 'if', 'unless', 'with', 'block', 'contentFor', 'each', 'lookup','randomElement'
...
rules = {
    ...
    'GS002-DISQUS-ID': {
        ...
        This results in Disqus comments not loading on Ghost 1.0.0 posts which were imported from earlier versions.
        To resolve this, we've added a new <code>{{comment_id}}</code> helper that will output the old ID
        for posts that have been imported from earlier versions, and the new ID for new posts.
    ...
    'GS002-ID-HELPER': {
        ...
        details: oneLineTrim`The output of <code>{{id}}</code> has changed in v1.0.0 from an incremental ID to an ObjectID.
        In v1.0.0 we added a new <code>{{comment_id}}</code> helper that will output the old ID
        for posts that have been imported from earlier versions, and the new ID for new posts.
    ...
    'GS005-TPL-ERR': {
        ...
        See the full list of available helpers <a href="${docsBaseUrl}" target=_blank>here</a>.`
    },
    'GS010-PJ-REQ': {
        level: 'error',
    ...
    'GS040-GH-REQ': {
        ...
        details: oneLineTrim`The <code>{{ghost_head}}</code> helper should be present in your theme. It outputs many useful things, such as <a href="https://help.ghost.org/hc/en-us/articles/223403488-Code-Injection" target=_blank>"code injection" scripts</a>, structured data, canonical links, meta description etc.<br>
    ...
    'GS040-GF-REQ': {
        ...
        details: oneLineTrim`The <code>{{ghost_foot}}</code> helper should be present in your theme. It outputs scripts as saved in <a href="https://help.ghost.org/hc/en-us/articles/223403488-Code-Injection" target=_blank>"code injection" scripts</a>.<br>
